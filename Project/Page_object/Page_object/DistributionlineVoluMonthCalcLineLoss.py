...
class DistributionlineVoluMonthCalcLineLoss():
    def __init__(self,driver):
        #self.driver = webdriver.Chrome()
        self.driver = driver
        self.datafile = GetProjectFilePath() + u"\Page_object\Data\DistributionlineVoluMonthCalcLineLoss.yaml"
        with open(self.datafile) as datafile:
            self.data = yaml.load(datafile)
        self.datamsg = self.data['DistributionlineVoluMonthCalcLineLoss']
    ...
    def DistributionlineVoluMonthCalcLineLoss_Fun(self):
        # 点击理论线损树
        time.sleep(1)
        ...
        #self.ExportExecl = ExcelData(u"C:\\Users\\"+GetUsername()+u"\\Downloads\\配电线路容量法导线损耗报表（月）.xls", u"配电线路导线损耗报表（月）")
        self.ExportExecl = ExcelData(u"C:\\Users\\" + GetUsername() + u"\\Downloads\\配电线路导线损耗报表（月）.xls",
                                     u"配电线路导线损耗报表（月）")
...
if __name__ == "__main__":
    try:
        ...
        Try = DistributionlineVoluMonthCalcLineLoss(driver)
        result = Try.DistributionlineVoluMonthCalcLineLoss_Fun()
        print result
    finally:
        driver.quit()
...
时间          ：2019/1/2 14:56
当前项目名称  ：Auto
功能          ：配电线路综合分析日报表-变压器损耗电量追溯（配电网高损配变分析报表）核对
-------------------------------------漂亮的分割线----------------------------------------------'''
import time
import yaml
import Login
...
class DistributionlineComAHlTraDay:
    def __init__(self,driver):
        '''变压器损耗电量追溯结果报表核对'''
        self.driver=driver
        self.ProjectFilePath=GetProjectFilePath()
        self.Page_object_data_file=open(self.ProjectFilePath+"\Page_object\Data\DistributionlineComAHlTraDay.yaml")
        ...
        #切换到配电网高损配变分析报表（日）frame中
        self.Disttransformerloss_frame=self.Data['Disttransformerloss_frame']
        #点击变压器损耗电量追溯页面的导出
        self.Export_ID=self.Data['Export_ID']

    def DistributionlineComAHlTraDay_Fun(self):
        '''变压器损耗电量追溯结果报表核对'''
        #点击线损分析
        ...
        self.driver.implicitly_wait(15)
        #切换到配电网高损配变分析报表（日）frame中
        self.driver.switch_to.frame(self.Disttransformerloss_frame)
        self.driver.implicitly_wait(7)
        #点击变压器损耗追溯页面的导出
        self.driver.find_element_by_id(self.Export_ID).click()
        time.sleep(7)
        self.data_file=u"C:\\Users\\Admin\Downloads\配电网高损配变分析报表(日).xls"
...
if __name__=="__main__":
    ...
    Try1=DistributionlineComAHlTraDay(driver)
    Try1.DistributionlineComAHlTraDay_Fun()
    time.sleep(10)
    driver.quit()

...
作者          ：王禹
IDE           ：PyCharm
时间          ：2018/10/19 0019 上午 9:43
当前项目名称  ：Auto
功能          ：理论线损容量法日计算，计算日期为2014-8-1
-------------------------------------漂亮的分割线----------------------------------------------
'''
...
class DistributionlineVoluDayCalc:
    '''620kv日容量法计算页面封装，封装的计算日期为2014-8-1'''
    def __init__(self,driver):
        #self.driver = webdriver.Chrome()
        self.driver = driver
        self.datafile = GetProjectFilePath() +u"\Page_object\Data\DistributionlineVoluDayCalc.yaml"
        with open(self.datafile) as datafile:
            self.data = yaml.load(datafile)
        self.datamsg = self.data['DistributionlineVoluDayCalc']
        ...
        #计算类型为日计算
        self.CalcDataType_css = self.datamsg["CalcDataType_css"]
        #计算日期为2014-8-1
        self.CalcDataInput_css = self.datamsg["CalcDataInput_css"]
        # 计算按钮定位新
    ...
    def DistributionlineVoluDayCalc_Fun(self,CalcDate = "2014-08-01"):
        '''访问配电线路620kv计算界面，选择计算线路以及计算方法以及计算日的时间并执行计算，如果成功返回True，失败False'''
        #点击树结构中的理论计算
        ...
        #计算类型为日计算
        self.driver.find_element_by_css_selector(self.CalcDataType_css).click()
        time.sleep(1)
        #计算日期为2014-8-1
        self.driver.find_element_by_css_selector(self.CalcDataInput_css).clear()
        self.driver.find_element_by_css_selector(self.CalcDataInput_css).send_keys(CalcDate)
        self.driver.find_element_by_css_selector(self.CalcDataInput_css).send_keys(Keys.TAB)
        #点击计算按钮
        time.sleep(1)
...
if __name__ == '__main__':
    ...
    test1 = DistributionlineVoluDayCalc(driver)
    print test1.DistributionlineVoluDayCalc_Fun()
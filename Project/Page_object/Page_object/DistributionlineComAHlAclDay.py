...
时间          ：2019/1/2 14:07
当前项目名称  ：Auto
功能          ：配电线路综合分析日报表-导线损耗电量追溯（配电线路高损导线分析报表）核对
-------------------------------------漂亮的分割线----------------------------------------------'''
import time
import yaml
import Login
...
class DistributionlineComAHlAclDay:
    def __init__(self,driver):
        '''导线损耗电量追溯结果报表核对'''
        self.driver=driver
        self.ProjectFilePath=GetProjectFilePath()
        self.Page_object_data_file=open(self.ProjectFilePath+"\Page_object\Data\DistributionlineComAHlAclDay.yaml")
        self.Page_data=yaml.load(self.Page_object_data_file)
        self.Page_object_data_file.close()
        self.Data=self.Page_data['DistributionlineComAHlAclDay']
        ...
        #点击导线损耗电量追溯
        self.Distaclinesegeloss_Xpath=self.Data['Distaclinesegeloss_Xpath']
        #切换到配电线路高损导线分析报表frame中
        self.Distaclinesegeloss_frame=self.Data['Distaclinesegeloss_frame']
        #点击导线损耗电量追溯页面的导出
        self.Export_ID=self.Data['Export_ID']

    def DistributionlineComAHlAclDay_Fun(self):
        '''导线损耗电量追溯结果报表核对'''
        #点击线损分析
        ...
        time.sleep(7)
        #点击导线损耗电量追溯
        self.driver.find_element_by_xpath(self.Distaclinesegeloss_Xpath).click()
        time.sleep(3)
        #跳出配电线路线损分析的frame
        self.driver.switch_to.default_content()
        self.driver.implicitly_wait(15)
        #切换到配电线路高损导线分析报表frame中
        self.driver.switch_to.frame(self.Distaclinesegeloss_frame)
        self.driver.implicitly_wait(7)
        #点击导线损耗追溯页面的导出
        self.driver.find_element_by_id(self.Export_ID).click()
        time.sleep(5)
        self.data_file=u"C:\\Users\\Admin\Downloads\配电线路高损导线分析报表.xls"
...
if __name__=="__main__":
    ...
    Try1=DistributionlineComAHlAclDay(driver)
    Try1.DistributionlineComAHlAclDay_Fun()
    time.sleep(10)
    driver.quit()

...
class DistributionlineOverloadTranDay:
    def __init__(self,driver):
        '''配电线路综合分析-重载变追溯结果核对'''
        self.driver=driver
        self.ProjectPathFile=GetProjectFilePath()
        self.Page_object_data_file=open(self.ProjectPathFile+"\Page_object\Data\DistributionlineOverloadTranDay.yaml")
        self.Page_data=yaml.load(self.Page_object_data_file)
        self.Page_object_data_file.close()
        self.Data=self.Page_data['DistributionlineOverloadTranDay']
        ...
        #点击重载变台数
        self.OverLoadTranNum_Xpath=self.Data['OverLoadTranNum_Xpath']
        #切换到配变损耗组成分析-重载变frame中
        self.OverLoadTranNum_frame=self.Data['OverLoadTranNum_frame']
        #点击左下角的导出按钮
        self.Export_ID=self.Data['Export_ID']

    def DistributionlineOverloadTranDay_Fun(self):
        '''配电线路综合分析-重载变追溯结果核对'''
        #点击线损分析
        ...
        #点击重载变台数
        self.driver.find_element_by_xpath(self.OverLoadTranNum_Xpath).click()
        time.sleep(3)
        self.driver.switch_to.parent_frame()  ##跳出配变运行综合分析（日）frame
        time.sleep(5)
        #切换到配变损耗组成分析-重载变frame中
        self.driver.switch_to.frame(self.OverLoadTranNum_frame)
        time.sleep(5)
        #点击左下角的导出按钮
        self.driver.find_element_by_id(self.Export_ID).click()
        time.sleep(4)
        self.data_file=u"C:\\Users\\Admin\Downloads\配变损耗组成分析(日)-重载变.xls"
...
if __name__=="__main__":
    ...
    Try1=DistributionlineOverloadTranDay(driver)
    Try1.DistributionlineOverloadTranDay_Fun()
    time.sleep(10)
    driver.quit()